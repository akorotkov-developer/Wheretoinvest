...
class Dumphper
{
    ...
    /** Text encoding, needed to escape stings **/
    static $escape_keys = false;
    /** Should array keys be escaped (slow) **/
    static $max_showw_depth = 1;
    /** Defines how many nested levels will be expanded by default **/
    ...
    {
        if ($type == "string" && self::$depth == 1 && preg_match("#<.*?>#is", html_entity_decode($value)))
            echo '<span class="dumphper-value dumphper-' . $type . '" title="' . $type . '"><pre>' . preg_replace("#[\r\n]+#i", "<br>", $value) . '</pre></span>';
        else
