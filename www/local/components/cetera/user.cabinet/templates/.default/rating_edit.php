...
<form action="" method="post" enctype="multipart/form-data" class="b-form js-form">
        ...
        <? foreach ($rating as $item): ?>
            <div class="row edit js-item">
                <div class="edit__first medium-4 small-12 columns">
                    ...
                    <input type="text" name="rating[<?= $item["ID"] ?>][agency]" class="edit__input"
                           value="<?= $item["UF_AGENCY"] ?>" required>
                </div>
                <div class="edit__second medium-3 small-12 columns">
                    <label for="#" class="edit__label<? if (!$first): ?> show-for-small-only<? endif; ?>">
                        Рейтинг
                    </label>
                    <input type="text" name="rating[<?= $item["ID"] ?>][rating]" class="edit__input"
                           value="<?= $item["UF_RATING"] ?>" required>
                </div>
                <div class="edit__third medium-4 small-10 columns">
                    <label for="#" class="edit__label<? if (!$first): ?> show-for-small-only<? endif; ?>">
                        Дата получения
                    </label>
                    <input type="date" name="rating[<?= $item["ID"] ?>][date]" class="edit__input"
                           value="<?= !empty($item["UF_DATE"]) ? date("Y-m-d", strtotime($item["UF_DATE"])) : "" ?>"
                           required>
                </div>
                <div class="edit__fourth medium-1 small-2 columns">
    ...
    <div class="row">
        <div class="columns">
            ...
            <button type="submit" class="content__submit<? if (!count($rating)): ?> hide<? endif; ?>">Сохранить</button>
        </div>
    </div>
</form>
